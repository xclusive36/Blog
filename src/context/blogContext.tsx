...
On the left, you initially see a set of files and folders. These are the files and folders that make up the source code of your new application. Vite has preconfigured and set this folder structure this way to optimize its compilation during a build.`,
    content: `Lets focus on a couple of files briefly, the file vite.config.ts is the primary config file for Vite. It defines the type of environment the application will use. In our case React. The file tsconfig.json is the primary compiler configuration file. It specifies the root files and options required to compile the project. We shouldn't need to make any modifications to these files.
    
The package.json file is a the primary document that stores information about the project. It stores the meta data of the project such as the name, version, scripts, dependencies, and a license. It is stored in JSON data format. This file will be modified by us both directly and indirectly. Its paired file the package-lock. json is a file that is automatically generated by NPM when a package is installed. It records the exact version of every installed dependency, including its sub-dependencies and their versions. We will not be modifying this file directly, only indirectly. Specifically when we add a new NPM package to our project, the package-lock.json gets updated accordingly.
    
The folders: The node_modules folder is folder created by NPM to store all of the NPM packages that are necessary for the project to function. We can leave this folder alone. Feel free to browse it. It's a rather large folder and will take up the bulk of the size of our project source code. It can get a few gigs in size or larger in very detailed projects.
